...
from translator_utils import (
    is_russian, sanitize_name, needs_content_based_name,
    safe_exists, safe_is_file, safe_is_dir, detect_language
)
...
class WorkspaceRUENTranslator:
    def __init__(self, root_path: str, **kwargs):
        ...
        self.text_extensions = {".txt", ".md", ".log", ".rst", ".cfg", ".conf", ".tex", ".yaml", ".yml", ".xml", ".html"}
        ...
        self.rename_proc = RenameProcessor(self.client, self._lock, self._unique_path, auto_detect=self.auto_detect, target_lang=self.target_lang)

        self.stats = {k: 0 for k in ['dirs_created', 'dirs_existing_merged', 'items_moved', 'files_renamed', 'files_content_translated', 'files_content_skipped', 'sidecars_written', 'files_content_renamed']}

    def _stats_inc(self, key):
        with self._lock: self.stats[key] += 1

    def _unique_path(self, path: Path) -> Path:
        with self._lock:
            if not safe_exists(path): return path
            suf, stem = path.suffix, path.stem
            while len(os.fsencode(stem)) > 250: stem = stem[:-1].rstrip()
            ...
            while True:
                ...
                if not safe_exists(new_p): return new_p
                i += 1

    def _maybe_rename_via_content(self, path: Path) -> None:
        if not needs_content_based_name(path.stem): return
        ...
        if self.auto_detect:
            ...
            if src_lang == self.target_lang: return # Already target language
        ...
        if not candidate or candidate == path.stem: return
    ...
    def _get_content_snippet(self, path: Path) -> Optional[str]:
        ...
        try:
            if suf in self.text_extensions or suf in {".yaml", ".yml", ".xml", ".html"}:
            ...
            if suf == ".json":
                import json
                return self._find_first_json_string(json.loads(path.read_text(encoding="utf-8", errors="ignore")))
                ...
                if suf == ".docx":
                    ...
                    if HAS_DOCX:
                        for p in DocxDocument(path).paragraphs:
                            if p.text.strip(): return p.text.strip()
    ...
    def _process_content_for_file(self, path: Path) -> None:
        if not safe_is_file(path): return
        ...
        if s in self.text_extensions: ok, t = self.text_handler.translate_text_inplace(path), "text"
        elif s == ".csv": ok, t = self.text_handler.translate_csv_inplace(path), "csv"
        elif s == ".json": ok, t = self.text_handler.translate_json_inplace(path), "json"
        elif s == ".docx": ok, t = self.office_handler.translate_docx_inplace(path), "docx"
        elif s == ".xlsx": ok, t = self.office_handler.translate_xlsx_inplace(path), "xlsx"
        elif s == ".pptx": ok, t = self.office_handler.translate_pptx_inplace(path), "pptx"
        elif self.translate_extract_sidecars and s in ({".pdf", ".vsd", ".vsdx"} | self.image_extensions):
        ...
        else: self._stats_inc('files_content_skipped')

    def _process_sidecar(self, path: Path, suf: str) -> None:
        text = None
        if suf == ".pdf":
            text = self.media_handler.extract_pdf_text(path)
            if not text or not is_russian(text): text = self.media_handler.extract_pdf_ocr_text(path)
        elif suf in (".vsd", ".vsdx"): text = self.media_handler.extract_vsd_text(path)
        elif suf in self.image_extensions: text = self.media_handler.extract_ocr_text(path)
    ...
    def run(self) -> None:
        ...
        if not self.upgrade_only:
            print("\nPASS 1: Renaming..."); self.rename_proc.process_dirs_recursive(self.root_path, self._stats_inc)
        if not self.rename_only:
            print("\nPASS 2: Upgrading..."); legacy = [p for p in self.root_path.rglob("*") if safe_is_file(p) and p.suffix.lower() in {".doc", ".xls", ".ppt", ".rtf", ".odt"}]
            upgraded = sum(1 for p in legacy if self.office_handler.upgrade_office_file(p, self._unique_path) != p)
            print(f"  Success: Upgraded {upgraded} legacy files")
        if not self.rename_only and not self.upgrade_only:
            print("\nPASS 3: Content..."); [self._process_content_for_file(p) for p in self.root_path.rglob("*") if safe_is_file(p) and not p.name.startswith(".")]
