...
from translator_utils import is_russian, sanitize_name, split_suffixes, safe_exists, safe_is_file, safe_is_dir, move_path, detect_language

class RenameProcessor:
    def __init__(self, client, lock, unique_path_func, auto_detect=False, target_lang='en'):
        self.client = client
        self.lock = lock
    ...
    def translate_filename(self, filename: str) -> str:
        ...
        new_stem = sanitize_name(self.client.translate(stem)) if self._should_translate(stem) else sanitize_name(stem)
        new_stem = new_stem or stem
        full = f"{new_stem}{suf}"
        while len(os.fsencode(full)) > 255:
            new_stem = new_stem[:-1].rstrip()
            if not new_stem: return filename[:255] if len(os.fsencode(filename)) > 255 else filename
            full = f"{new_stem}{suf}"
        return full

    def merge_dir_into(self, src_dir: Path, dst_dir: Path, stats_callback) -> None:
        try: dst_dir.mkdir(parents=True, exist_ok=True)
        except OSError: return
        try: items = list(src_dir.iterdir())
        except OSError: return
        for item in items:
            if safe_is_dir(item):
                ...
                if safe_exists(target): target = self.unique_path_func(target)
                if new_name != item.name: stats_callback('files_renamed')
                move_path(item, target)
                stats_callback('items_moved')

    def process_dirs_recursive(self, current: Path, stats_callback) -> None:
        try: items = sorted(current.iterdir())
        except OSError: return
        ...
        for d in (p for p in items if safe_is_dir(p)):
            ...
            translated = sanitize_name(self.client.translate(d.name)) if self._should_translate(d.name) else d.name
